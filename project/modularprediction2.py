...
# weekly avg, 
paramMaster.append(['8day_Volume_HighLow_Niave_noCoded_Trend_sSma7_SMA5'])
#paramMaster.append(['baseline'])
...
def dostuff(params):
    ...
    graphpath = 'ModelOutputs/'+ company+ '_' +params[0]
    print graphpath
    
    def preprocessData(currentStockID, otherdata, daysBack=daysBack, useVolume = useVolume, scaleVolume = scaleVolume, useHighLow=useHighLow, useSMA=useSMA):
        if useVolume == False:
            scaleVolume = False
        ...
        #sma
        if useSMA > 0:
            smadates = (stockData.iloc[:,0:1].values).flatten()
            smaclosing = (stockData.iloc[:,4:5].values).flatten()
            smaseries = pd.Series(smaclosing, smadates)
            sma = pd.rolling_mean(smaseries, useSMA)
            sma = np.reshape(sma.values, (len(sma),1))
            for i in range(useSMA):
                sma[i]= 0
            sma = sc.transform(sma)
            training_set_scaled = np.append(training_set_scaled, sma, axis=1)
    ...
    otherdata=[]
    #process the data
    otherdata.append(weeklyAverage)
    otherdata.append(sentimentmap)
    ...
    print training_set_scaled[-100:-95]
    ...
    thismean, thisnorm = displayAndSaveResults(True, real_stock_price, predicted_stock_price, stockname, graphpath)
...
for c in companies:
    for i in paramMaster:
        ...
        thismean, thisnorm = dostuff(i)
        
    themeans.append(thismean)
    thenorms.append(thisnorm)
    
print "means"
print np.mean(themeans)
print themeans
print ""

print "norms"
print np.mean(thenorms)
print thenorms
